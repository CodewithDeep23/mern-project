...
function SearchVideos() {
  ...
  if (videos?.totalDocs === 0) {
    return <EmptyVideo text={"Try searching something else"} />;
  }
  ...
  return (
    ...
    <>
      <div
        ...
        onClick={() => setFilterOpen((prev) => !prev)}
      >
        <span className="text-white dark:text-gray-200 hover:text-purple-500 dark:hover:text-purple-400">
          Filters
        </span>
        <FaFilter
          size={20}
          className="text-purple-500 hover:text-purple-800 dark:text-purple-400 dark:hover:text-purple-300"
        />
      </div>

      <div className="w-full text-white dark:text-gray-200">
        {filterOpen && (
          <div className="w-full absolute bg-transparent">
            <div className="max-w-sm border border-slate-800 dark:border-gray-600 rounded bg-[#222222] dark:bg-gray-900 fixed mx-auto z-50 inset-x-0 h-96 p-5">
              <h1 className="font-semibold text-lg">Search filters</h1>
              <IoCloseCircleOutline
                size={25}
                className="absolute right-5 top-5 cursor-pointer"
              ...
              />
              <table className="mt-4">
                <tr className="w-full text-start border-b border-gray-600 dark:border-gray-700">
                  <th>SortBy</th>
                </tr>
                <tr className="flex flex-col gap-2 text-slate-400 dark:text-gray-400 cursor-pointer">
                  <td onClick={() => handleSortParams("createdAt", "desc")}>
                    Upload date <span className="text-xs">(Latest)</span>
                  </td>
                  <td onClick={() => handleSortParams("createdAt", "asc")}>
        ...
        <div className="grid h-screen xl:grid-cols-3 sm:grid-cols-2 grid-cols-1 text-white dark:text-gray-200 overflow-y-scroll">
          {videos &&
            videos?.docs?.map((video) => (
              <VideoList
                key={video?._id}
