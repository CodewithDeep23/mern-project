...
function PlaylistVideoAtom({ video, playlistId, owner = false }) {
  ...
  function handleButtonClick() {
    if (isDeleted)
      dispatch(addVideoToPlaylist({ playlistId, videoId: video._id })).then(
        (res) => {
      ...
      <div className="w-full gap-x-4 sm:flex">
        ...
        <div className=" flex relative gap-x-2 px-2 sm:w-7/12 sm:px-0">
          ...
          <div className="w-full mt-2">
            {/* title */}
            <h6 className="mb-1 text-lg font-semibold sm:max-w-[75%]">
              {video.title}
            </h6>
            {/* views and date */}
          ...
          {owner && (
            <span className="absolute right-2 top-2">
              <button
                ...
                className="h-5 justify-self-end  text-xl mr-1"
              >
                {/* Remove */}
                {!isDeleted && (
                  <span
                    title="remove video"
                    className="h-4 text-red-400 hover:text-red-600"
                  >
                    {icons.emptyDelete}
                  </span>
                )}
