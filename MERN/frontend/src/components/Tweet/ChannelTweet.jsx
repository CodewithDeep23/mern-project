...
import { getUserTweet, createTweet } from "../../store/Slices/tweetSlice";
import {
  MyChannelEmptyTweet,
  TweetAtom,
  EmptyTweet
...
function ChannelTweets({ owner = false }) {
  ...
  const { status: authStatus, userData: currentUser } = useSelector(
    ({ auth }) => auth
  );
  ...
  useEffect(() => {
    ...
    if (!userId) return;
    dispatch(getUserTweet(userId)).then((res) => {
